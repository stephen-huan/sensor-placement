...
if __name__ == "__main__":
    ...
    plt.scatter(X[entropy, 0], X[entropy, 1], label="entropy",
                zorder=2, s=BIG_POINT, marker="D", color=lightblue)
    plt.scatter(X[mi, 0], X[mi, 1], label="mutual info",
                zorder=3, s=BIG_POINT, marker="s", color=rust)
    ...
    surf = ax.plot_surface(
        X[:, 0].reshape(n, n), X[:, 1].reshape(n, n), y.reshape(n, n),
        label="measurements", zorder=0, color=silver, alpha=0.5
    )
    # https://stackoverflow.com/questions/54994600/65554278#65554278
    surf._facecolors2d = surf._facecolor3d
    surf._edgecolors2d = surf._edgecolor3d
    ax.scatter(X[:, 0], X[:, 1], y, label="candidates",
               zorder=1, s=POINT_SIZE, color=silver)
    ax.scatter(X[entropy, 0], X[entropy, 1], y[entropy], label="entropy",
               zorder=2, s=BIG_POINT, marker="D", color=lightblue)
    ax.scatter(X[mi, 0], X[mi, 1], y[mi], label="mutual info",
               zorder=3, s=BIG_POINT, marker="s", color=rust)
    plt.legend()
