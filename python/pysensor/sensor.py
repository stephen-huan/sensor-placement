...
def solve_triangular(L: np.ndarray, b: np.ndarray,
                     lower=True, **kwargs) -> np.ndarray:
...
def chol_downdate(L: np.ndarray, u: np.ndarray, j: int=None) -> np.ndarray:
    """ Computes the rank-one downdate in-place, L -> chol(L L^T - u u^T). """
    if j is None: j = L.shape[0]
    for i in range(j):
        c1, c2 = L[i, i], u[i]
...
def entropy_naive(X: np.ndarray, kernel: Kernel, s: int) -> list:
...
def entropy_prechol(X: np.ndarray, kernel: Kernel, s: int) -> list:
    """ Returns a list of the most entropic points in X greedily. """
    # O(s*(n*s + s^2)) = O(n s^2)
    n = len(X)
    s = min(s, n)
...
def entropy_chol(X: np.ndarray, kernel: Kernel, s: int) -> list:
    """ Returns a list of the most entropic points in X greedily. """
    # O(s*(n*s + s^2)) = O(n s^2)
    n = len(X)
    s = min(s, n)
...
def mi_prec(X: np.ndarray, kernel: Kernel, s: int) -> list:
    """ Max mutual information between selected and non-selected points. """
    # O(n^3 + s*(n^2)) = O(n^3)
    n = len(X)
    # initialization
...
def mi_chol(X: np.ndarray, kernel: Kernel, s: int) -> list:
    """ Max mutual information between selected and non-selected points. """
    # O(n^3 + s*(n^2)) = O(n^3)
    n = len(X)
    # initialization
    ...
    L1 = np.zeros((n, s))
    L2 = np.flip(
        solve_triangular(np.linalg.cholesky(kernel(X[::-1])), np.identity(n))
    ).T
    cond_var1 = kernel.diag(X)
    # the full conditional of i corresponds to the ith diagonal in precision
